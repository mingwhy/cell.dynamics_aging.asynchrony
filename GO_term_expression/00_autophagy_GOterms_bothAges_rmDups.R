...
df.go.chaperone=df.go[grep('\\bautophagy\\b',ignore.case = t,df.go$GOterm),] 
df.go.chaperone=df.go.chaperone[-grep('chaperone',df.go.chaperone$GOterm),]

rownames(df.go.chaperone)=df.go.chaperone$GOid
dim(df.go.chaperone) #16 x 2
...
pdf('upset.plot_autophagy_GOterm.pdf',useDingbats = T)
print(p1)
dev.off()
...
output_file=paste0('select.tc.h5ad_result_goterms/mouse_male_GO_',pick.age,'_libNorm2_autophagy.rds');
output_file
...
pdf(paste0('GO_autophage_cell.lifespan_',pick.age,'_rmDups_onePage.pdf'),useDingbats = T,
