...
df=data.table::fread('../0708_TMS_male_3m_18m/sce.minCellCounts_gcl_per.mouse.txt')
df$cell.type=df$tc
df$dist=1-df$gcl;

tmp0=df %>% group_by(cell.type,age,mouse.id) %>% summarise(age.average.per.mouse=mean(dist))
df.summary=tmp0 %>% group_by(cell.type,age) %>% 
  summarise_at(vars(matches("age.average.per.mouse")), funs(mean=mean,median=median))
head(df.summary)
df.res1=df.summary

##############################################################
## median CV per age group, 3 vs 24
df=data.table::fread('../0708_TMS_male_3m_24m/sce.minCellCounts_gcl_per.mouse.txt')
df$cell.type=df$tc
df$dist=1-df$gcl;
...
df.res=df.res0 %>% group_by(tc,numeric.age) %>% summarise(CV=mean(median))
...
p1<-ggplot(df,aes(x=mean,y=CV,group=month,col=month))+
  ...
  xlab('Mean cell age')+ylab('Average cell-to-cell heterogeneity (1-GCL)')+#scale_x_log10()+
...
p3<-ggplot(df.rmdup2,aes(x=mean,y=CV.rm.dup,group=month,col=month))+
  ...
  xlab('Mean cell age (day)')+#ylab('Average cell-to-cell heterogeneity (1-Spearman\'s rho )')+#scale_x_log10()+
  ylab('Average (1-GCL)')+
  geom_point()+scale_color_manual(name='Month',values=mycols)+
...
pdf('mean.cell.age_obs.CV_male_gcl_per.mouse.pdf',useDingbats = T,width = 6,height = 5)
print(p3);
dev.off()
