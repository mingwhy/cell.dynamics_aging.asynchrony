...
##############################################################
## median value per age group, 3 vs 18
#df=readRDS('../0708_TMS_male_3m_18m/res_cell.cors.rds')
df=readRDS('../0708_TMS_male_3m_18m/res_cell.cors_per.mouse.rds')
df$cell.dist=1-df$cell.cors;

tmp0=df %>% group_by(cell.type,age,mouse.id) %>% summarise(age.average.per.mouse=median(cell.dist))
...
df=readRDS('../0708_TMS_male_3m_24m/res_cell.cors_per.mouse.rds')
df$cell.dist=1-df$cell.cors;
...
colnames(df.res0)[1]='tc'
df.res0$numeric.age=as.numeric(gsub('m','',df.res0$age))
sum(is.na(df.res0$age.average))

df.res=df.res0 %>% group_by(tc,numeric.age) %>% summarise(CV=mean(age.average,na.rm=T))
...
p1<-ggplot(df,aes(x=mean,y=CV,group=month,col=month))+
  ...
  xlab('Mean cell age')+ylab('Average cell-to-cell heterogeneity (1-Spearman\'s rho )')+#scale_x_log10()+
  geom_point()+scale_color_manual(name='Month',values=mycols)
p1


## took average for the same cell type
df.rmdup=df %>% group_by(numeric.age,Ron_tc) %>% mutate(CV.rm.dup=mean(CV,na.rm=T))
...
p3<-ggplot(df.rmdup2,aes(x=mean,y=CV.rm.dup,group=month,col=month))+
  ...
  xlab('Mean cell age (day)')+
  ylab('Average (1-Spearman\'s rho)')+
  geom_point()+scale_color_manual(name='Month',values=mycols)+
  stat_poly_eq(use_label(c("R2","p"))) 
p3


pdf('mean.cell.age_obs.CV_male_spearman.rho_per.mouse.pdf',useDingbats = T,width = 6,height = 5)
print(p3);
dev.off()
