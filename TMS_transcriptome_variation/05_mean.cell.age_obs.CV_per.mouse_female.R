...
DM.out=readRDS('../0708_TMS_female_3m_18m/MAST_nonDEgene_stats.rds')
...
p1<-ggplot(df,aes(x=mean,y=CV,group=month,col=month))+
  ...
  xlab('Mean cell age')+ylab('Average log10(CV)')+
...
pdf('mean.cell.age_obs.CV_female.pdf',useDingbats = T,width = 6,height = 7)
print(p3);
dev.off()
